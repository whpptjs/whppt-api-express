...
test('Should send forgot password email with saved user tokens', () => {
  ...
  const context = {
    whpptOptions: {
      emailTemplates: { forgotPassword: jest.fn(() => Promise.resolve('htmlString')) },
    },
    $env: { BASE_URL: 'http://test.com' },
    $security: {
      generateAccessToken: () =>
        Promise.resolve({ token: 'testToken', tokenExpiry: 'testExpiry' }),
    },
  ...
  return cmd.exec(context, { email }).then(response => {
    ...
    const [[forgotPasswordArgs]] =
      context.whpptOptions.emailTemplates.forgotPassword.mock.calls;
    ...
    expect(forgotPasswordArgs.resetLink).toBe(
      'http://test.com/?token=testToken&email=testEmail'
    );
  });
});
