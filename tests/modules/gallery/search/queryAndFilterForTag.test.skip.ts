...
test('query and filter for tags', () => {
  ...
  const args = { domainId: datatype.uuid(), type: 'image', filterTag: 'test', queryTags: ['test1', 'test2'] };
  ...
  return search.exec(context, args as SearchParams).then(() => {
    ...
    expect(collection.find.firstCall.firstArg).toStrictEqual({
      ...
      type: 'image',
      tags: { $and: [{ $elemMatch: { $eq: 'test' } }, { $in: ['test1', 'test2'] }] },
    });
  });
});
