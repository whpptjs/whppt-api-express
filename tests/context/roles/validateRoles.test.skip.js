...
describe('validate roles:', () => {
  ...
  test('is admin roles should be included in the required roles', () => {
    ...
    const context = {
      $mongo: { $db: { collection: sinon.stub() } },
      $env: { draft: true },
    };
    const find = sinon.stub();
    context.$mongo.$db.collection.withArgs('roles').returns({ find });
    find
      .withArgs({ _id: { $in: user.roles } })
      .returns({ toArray: () => Promise.resolve(userRoles) });
    find
      .withArgs({ admin: true }, { _id: true })
      .returns({ toArray: () => Promise.resolve(adminRoles) });
