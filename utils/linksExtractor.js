...
module.exports = function linksExtractor(pageType, page) {
  let links = pageType.extractLinks ? map(pageType.extractLinks(page), link => ({ _id: $id(), parentId: page._id, slug: page.slug, href: link, type: 'link' })) : [];
  ...
  forEach(contentSections, contentSection => {
    forEach(page[contentSection], componentData => {
      ...
      const _componentLinks = map(componentLinks, cl => ({ _id: $id(), parentId: page._id, slug: page.slug, href: cl, type: 'link' }));
