...
module.exports = {
  exec({ $mongo: { $unpublish, $db, $startTransaction, $record }, $publishing }, { _id, collection, user }) {
    ...
    return $startTransaction(async session => {
      await $db.collection(collection).updateOne({ _id }, { $set: { published: false } }, { session });
